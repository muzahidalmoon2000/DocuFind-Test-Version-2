...
def search_all_files(token, query):
    ...
    # Only search in specific SharePoint sites
    sharepoint_sites = [
        ("elpiscapitalllc.sharepoint.com", "/sites/Elpis-BA3-Test"),
        ("elpiscapitalllc.sharepoint.com", "/sites/Elpis-BA3-Prod"),
        ("elpiscapitalllc.sharepoint.com", "/sites/Elpis-BA3-DEV"),
    ]

    for hostname, path in sharepoint_sites:
        site_url = f"https://graph.microsoft.com/v1.0/sites/{hostname}:{path}"
        site_res = retry_request(site_url, headers)
        if site_res.status_code != 200:
            logging.warning(f"⚠️ Could not resolve site {hostname}{path}")
            continue
